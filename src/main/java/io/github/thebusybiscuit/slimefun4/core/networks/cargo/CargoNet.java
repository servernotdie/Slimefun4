...
public class CargoNet extends AbstractItemNetwork implements HologramOwner {
    ...
    public void tick(@Nonnull Block b, SlimefunBlockData blockData) {
        if (!regulator.equals(b.getLocation())) {
            updateHologram(b, "&4Phát hiện nhiều bộ điều tiết mạng vận chuyển gần đây", blockData::isPendingRemove);
            return;
        }
        ...
        if (connectorNodes.isEmpty() && terminusNodes.isEmpty()) {
            updateHologram(b, "&cKhông tìm thấy nút mạng vận chuyển gần đó", blockData::isPendingRemove);
        } else {
            updateHologram(b, "&7Trạng thái: &a&lĐã kết nối", blockData::isPendingRemove);
