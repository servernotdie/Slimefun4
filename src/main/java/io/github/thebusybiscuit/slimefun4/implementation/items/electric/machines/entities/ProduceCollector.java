...
public class ProduceCollector extends AContainer implements RecipeDisplayItem {
    ...
    public @Nonnull List<ItemStack> getDisplayRecipes() {
        ...
        displayRecipes.add(new CustomItemStack(Material.BUCKET, null, "&fCần có &bBò &fở gần mới sản xuất"));
        displayRecipes.add(new ItemStack(Material.MILK_BUCKET));

        if (Slimefun.getMinecraftVersion().isAtLeast(MinecraftVersion.MINECRAFT_1_17)) {
            displayRecipes.add(new CustomItemStack(Material.BUCKET, null, "&fCần có &bDê &fở gần mới sản xuất"));
            displayRecipes.add(new ItemStack(Material.MILK_BUCKET));
        }

        displayRecipes.add(new CustomItemStack(Material.BOWL, null, "&fCần có &bMooshroom &fở gần mới sản xuất"));
    ...
    private boolean isAnimalNearby(Block b, Predicate<LivingEntity> predicate) {
        ...
        return !Slimefun.getNearbyEntities(b.getLocation(), radius, radius, radius, n -> isValidAnimal(n, predicate))
                .isEmpty();
    }
