...
public class ProgrammableAndroid extends SlimefunItem
    ...
    public ProgrammableAndroid(
        ...
        new UniversalMenuPreset(getId(), "Robot Có Thể Lập Trình") {
    ...
    public void openScript(Player p, SlimefunUniversalBlockData uniData, String sourceCode) {
        ...
        menu.addItem(
                ...
                new CustomItemStack(
                        ...
                        "",
                        "&7\u21E8 &eChuột trái &7quay lại bảng điều khiển robot"));
        ...
        for (int i = 1; i < script.length; i++) {
            ...
            if (i == script.length - 1) {
                ...
                if (hasFreeSlot) {
                    menu.addItem(i, new CustomItemStack(HeadTexture.SCRIPT_NEW.getAsItemStack(), "&7> Thêm lệnh mới"));
                                        ...
                                        .getMessage(
                                                ...
                                                "android.scripts.instructions."
                                                        + Instruction.valueOf(script[i])
                                                                .name()),
                                "",
                                "&7\u21E8 &eChuột trái &7chỉnh sửa",
                                "&7\u21E8 &eChuột phải &7xóa",
                                "&7\u21E8 &eShift + Chuột phải &7sao chép"));
                menu.addMenuClickHandler(i, (pl, slot, item, action) -> {
    ...
    protected void openScriptDownloader(Player p, SlimefunUniversalBlockData uniData, int page) {
        ChestMenu menu = new ChestMenu("Script Robot");
    ...
    public void openScriptEditor(Player p, SlimefunUniversalBlockData uniData) {
        ...
        menu.addItem(
                ...
                new CustomItemStack(
                        ...
                        "&6> Tải xuống script",
                        "",
                        "&eTải xuống script được tải lên bởi người chơi khác từ máy chủ",
                        "&eCó thể dùng ngay, hoặc chỉnh sửa trước khi sử dụng"));
    ...
    protected void editInstruction(Player p, SlimefunUniversalBlockData uniData, String[] script, int index) {
        ...
        menu.addItem(
                9,
                new CustomItemStack(HeadTexture.SCRIPT_PAUSE.getAsItemStack(), "&fKhông làm gì cả"),
                (pl, slot, item, action) -> {
                    String code = deleteInstruction(script, index);
                    setScript(uniData, code);
    ...
    public List<ItemStack> getDisplayRecipes() {
        ...
        for (MachineFuel fuel : fuelTypes) {
            ...
            lore.add(ChatColors.color("&8\u21E8 &7Còn lại " + NumberUtils.getTimeLeft(fuel.getTicks() / 2)));
            im.setLore(lore);
            item.setItemMeta(im);
            list.add(item);
        }
