...
public class TickerTask implements Runnable {
    ...
    private void reportErrors(Location l, SlimefunItem item, Throwable x) {
        ...
        if (errors == 1) {
            ...
            Slimefun.logger()
                    .log(
                            Level.SEVERE,
                            "Đã xảy ra nhiều lỗi trong 4 Tick vừa qua, máy móc tương ứng với khối này đã bị vô hiệu hóa.");
            Slimefun.logger()
                    .log(Level.SEVERE, "Vui lòng xem chi tiết lỗi trong thư mục /plugins/Slimefun/error-reports/.");
            Slimefun.logger()
                    .log(
                            Level.SEVERE,
                            "Nếu bạn cần trợ giúp, vui lòng gửi file báo cáo lỗi trên cho người khác, thay vì chụp màn hình cửa sổ này");
    ...
    /**
     * Trả về một {@link Map} <strong>chỉ đọc</strong> cho chunk đã cho
     * biểu thị tập hợp {@link Location vị trí} Tick của các vật phẩm
     * có thuộc tính {@link UniversalBlock} trong mỗi {@link ChunkPosition}.
     *
     * {@link Location} có trong đó có thể là {@link Chunk} đã tải hoặc chưa tải
     *
     * @param chunk
     *            {@link Chunk}
     *
     * @return {@link Map} chỉ đọc chứa tất cả {@link TickLocation vị trí} Tick của máy móc
     */
    @Nonnull
