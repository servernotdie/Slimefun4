...
public final class Script {
    ...
    ItemStack getAsItemStack(@Nonnull ProgrammableAndroid android, @Nonnull Player p) {
        ...
        lore.add("&7Tác giả &f" + getAuthor());
        lore.add("");
        lore.add("&7Lượt tải: &f" + getDownloads());
        lore.add("&7Đánh giá: " + getScriptRatingPercentage());
        lore.add("&a" + getUpvotes() + " \u263A &7| &4\u2639 " + getDownvotes());
        lore.add("");
        lore.add("&eChuột trái &fTải xuống script");
        lore.add("&4(Sẽ ghi đè script hiện có của bạn!)");

        if (canRate(p)) {
            lore.add("");
            lore.add("&eShift + Chuột trái &fĐánh giá tốt");
            lore.add("&eShift + Chuột phải &fĐánh giá xấu");
        }
